...
const acceptAppForm = async () => {
    try {
        const response = await fetch(`${backendEnvVar}/secretary/appform/pending/${appFormIdRef.value}/accept`, {
            method: 'POST',
            headers: {
...
const declineAppForm = async () => {
    try {
        const response = await fetch(`${backendEnvVar}/secretary/appform/pending/${appFormIdRef.value}/decline`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
