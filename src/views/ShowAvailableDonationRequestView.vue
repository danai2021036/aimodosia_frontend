...
const urlRef = computed(() => {
    return  '{{backendEnvVar}}'+'/aimodotis/donationrequests/' + userId.value;
});
...
const acceptDonationRequest = async () => {
    try {
        const response = await fetch(`${backendEnvVar}/aimodotis/donationrequests/${userId.value}/${donationRequestIdRef.value}/accept`, {
            method: 'POST',
            headers: {
